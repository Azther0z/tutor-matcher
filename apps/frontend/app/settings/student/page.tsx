...
export default function StudentSettingsPage() {
    ...
    fetch("/api/profiles/me/student", {
      ...
      .then(async (response) => {
        if (response.status === 404) {
          // Onboarding not completed yet: leave the form blank, same as today.
          return;
        }
        if (!response.ok) {
        ...
        setDuration(data.preferredDurationMinutes ? String(data.preferredDurationMinutes) : "");
        setSelectedAreas(data.learningAreas ?? []);
      })
      .catch((error: unknown) => {
        if ((error as DOMException)?.name === "AbortError") return;
        setMessage("We could not load your saved preferences. Please refresh and try again.");
      });
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    const token = localStorage.getItem("authToken");
    if (!token) {
      setMessage("Please log in before saving your Student profile.");
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        setMessage(data?.message ?? "Could not save your Student profile.");
        return;
      }
