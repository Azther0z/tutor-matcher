...
| BOOK-3  | ✅     | Cancel within policy is real, with an eligible refund to the wallet. Reschedule was decided in scope and is implemented alongside cancel, for the Student.                                                                                                                                                                                                                                  |
| BOOK-4  | ✅     | Differentiated from BOOK-3, resolving the prior duplicate-by-title concern: a Tutor-initiated cancellation is always a full refund with no fee, while a Student-initiated late cancellation still applies the policy rate. The Tutor can also reschedule, under the same 24-hour window as the Student. "Both parties are notified" is deferred to MSG-2, which owns notification delivery. |
