...
This is the canonical authored schema reference. Its database concepts are
also grounded in the immutable [Final Report](sources/tutor-matcher-final-report.md);
the implemented Prisma schema and migrations must be reconciled with it before
either is treated as authoritative.
