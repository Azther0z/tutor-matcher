...
| `just db-up` / `just db-down` | start (wait for healthy) / stop the Postgres container | `docker compose up -d --wait postgres` / `stop postgres`. |
| `just up` / `just down` | build and start / stop the Postgres + backend + frontend Compose stack | `just up` prints the local URLs (frontend `http://localhost:3000`, backend `http://localhost:8000`). |
| `just restart` / `just logs` / `just status` | restart / stream logs / show Compose service status | |
